...
con = db2.connect("DATABASE=bludb;HOSTNAME=6667d8e9-9d4d-4ccb-ba32-21da3bb5aafc.c1ogj3sd0tgtu0lqde00.databases.appdomain.cloud;PORT=30376;SECURITY=SSL;SSLServerCertificate=DigiCertGlobalRootCA.crt;UID=tbl66129;PWD=LPzyHjRGcmXprQoN",'','')
...
@app.route('/retservices',methods =['GET', 'POST'])
def retservices():
    return render_template('retservices.html');
    ...
    if request.method == 'POST' :
        ...
        if option=="Retailer":
            ...
            if account:
                ...
                session['id'] = account['RETID']
                userid=  account['RETID']
                session['username'] = account['EMAIL']
                msg = 'Logged in successfully !'
                print("userid:",userid)
                print(session['loggedin'])
                return redirect(url_for('retservices'))
                #return render_template('retservices.html', msg = msg)
            else:
            ...
            if account:
                #print("hi")
                session['loggedin'] = True
                session['id'] = account['CUSTID']
                userid=  account['CUSTID']
                session['username'] = account['EMAIL']
                msg = 'Logged in successfully !'
                print("userid:",userid)
                print(session['loggedin'])
...
def vadd():
    ...
    if request.method == "POST":
        ...
        if account:
            ...
            return redirect(url_for('vdisplay'))
            #return render_template('vdisplay.html')
...
def vupdate():
    ...
    if request.method == "POST":
        ...
        if account:
            ...
            return redirect(url_for('vdisplay'))
            #return render_template('view_vendor_DB.html')
...
def padd():
    ...
    if request.method == "POST":
        ...
        if account:
            ...
            #return render_template('view_products_DB.html')
...
def pdisplay():
    cur.execute("SELECT * FROM TBL66129.PRODUCTS")
    account = cur.fetchall()
    return render_template('view_products_DB.html',account=account)

@app.route('/vendordisplay')
def vdisplay():
    cur.execute("SELECT * FROM TBL66129.VENDORS")
    account = cur.fetchall()
    return render_template('view_vendor_DB.html',account=account)

@app.route('/custdisplay')
def cdisplay():
    cur.execute("SELECT * FROM TBL66129.CUSTOMER")
    account = cur.fetchall()
    return render_template('view_customers_DB.html',account=account)

@app.route('/retdisplay')
def rdisplay():
    cur.execute("SELECT * FROM TBL66129.RETAILER")
    account = cur.fetchall()
    return render_template('view_ret_DB.html',account=account)

@app.route('/orderdisplay')
def odisplay():
    cur.execute("SELECT * FROM TBL66129.ORDER")
    account = cur.fetchall()
    return render_template('view_orders_DB.html',account=account)
...
def plorder():
    ...
    if request.method == "POST":
        ...
        if account:
            ...
            insert_sql = "INSERT INTO ORDERS (CUSTID,PRODUCTID,PRODUCT_NAME,PRICE,DOP,TOP ,PHONE,QUANTITY) VALUES (?,?,?,?,?,?,?,?)"
            #(VENDOR_ID,VENDOR_NAME,CATEGORY,PRODUCTID)
            prep_stmt = ibm_db.prepare(conn, insert_sql)
            ...
            ibm_db.bind_param(prep_stmt, 4, pr)
            ibm_db.bind_param(prep_stmt, 5, dop)
            ibm_db.bind_param(prep_stmt, 6, top)
            ibm_db.bind_param(prep_stmt, 7, phone)
            ibm_db.bind_param(prep_stmt, 8, qty)
            ibm_db.execute(prep_stmt)
            ...
            print(account)
            if account:
                insert_sql = "UPDATE TBL66129.PRODUCTS SET QUANTITY = QUANTITY-? WHERE PRODUCTID = ?"
                prep_stmt = ibm_db.prepare(conn, insert_sql)
                ibm_db.bind_param(prep_stmt, 1, qty)
                ibm_db.bind_param(prep_stmt, 2, pid)
                ibm_db.execute(prep_stmt)
                msg = 'Successfully updated !'
                sqll="SELECT QUANTITY FROM TBL66129.PRODUCTS WHERE PRODUCTID=?"
                stmt = ibm_db.prepare(conn, sql)
                ibm_db.bind_param(stmt,1,pid)
                ibm_db.execute(stmt)
                account = ibm_db.fetch_row(stmt)
                #result = [dict([a, int(x)] for a, x in b.items()) for b in account]
                if  int(account) < 4:
                    mailfunc()
    ...
    return render_template('place_order.html',msg=msg)
